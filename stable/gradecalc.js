(() => {
  ...
  function subjectMetrics(subject){
    ...
    const projectedFinal = projectedBase === null ? null : clamp(projectedBase + bonusPercentPoints, 0, 100);
  ...
  function subjectFinalForStats(subject){
    ...
    if (subject.status === "completed" && Number.isFinite(Number(subject.finalRecordedPercent))) {
      ...
      const bonusPoints = subjectBonusEnabled(subject) ? clamp(Number(subject.courseBonusPercentPoints) || 0, 0, 10) : 0;
      return clamp(base + (bonusPoints * 10), 0, 100);
    }
  ...
  function displayProgress(metrics){
    ...
    if (scale === "ten"){
      ...
      return displayGrade(clamp(p + bonusPct, 0, 100));
    }
    ...
    return `
      ...
      <div class="gc-cols" style="margin-top:10px">
        <div>
          ...
          <div class="gc-check">
            <input class="gc-bonus-toggle" type="checkbox" id="bonusToggle-${escapeAttr(subject.id)}" data-sub-bonus-enabled ${bonusEnabledVal ? "checked" : ""} aria-label="enable bonus points" />
            <label class="gc-check-box" for="bonusToggle-${escapeAttr(subject.id)}" aria-hidden="true"></label>
          </div>
        </div>
      </div>
